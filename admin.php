...
function handleUpload($fileField)
{
    ...
    $optResult = optimizeImage($dest, 85);
    $optimized = isset($optResult['success']) && $optResult['success'];
    return ['name' => $imageName, 'optimized' => $optimized];
}
...
function handleMultiUpload($fileField)
{
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        // Optimize after upload
        $optResult = optimizeImage($dest, 85);
        // Optionally, you could track which images were optimized
        $saved[] = $imageName;
    }
...
// Image optimizer function with Imagick support
function optimizeImage($imagePath, $quality = 85)
{
    ...
    // Prefer Imagick if available and defined
    if (extension_loaded('imagick') && class_exists('Imagick')) {
        try {
            $img = new Imagick($imagePath);
            // Convert to WebP if supported, else JPEG
            $formats = array_map('strtolower', Imagick::queryFormats());
            $targetFormat = in_array('webp', $formats) ? 'webp' : 'jpeg';
            $optimizedPath = preg_replace('/\.[a-z0-9]+$/i', '_optimized.' . $targetFormat, $imagePath);
            $img->setImageFormat($targetFormat);
            ...
            <aside class="sidebar">
                ...
                <nav class="divide-y">
                    ...
                    <a href="admin.php?page=image-converter" class="sidebar-link <?php echo $page === 'image-converter' ? 'active' : ''; ?>">
                        <i class="fa-solid fa-right-left"></i> Image Converter
                    </a>
                ...
                <?php if ($page === 'image-converter'): ?>
                    <div class="card p-6">
                        <div class="flex items-center gap-2 mb-4 pb-4 border-b-2 border-gray-200">
                            <i class="fa-solid fa-right-left text-2xl text-[#00811F]"></i>
                            <h2 class="text-2xl font-bold">Image Converter</h2>
                        </div>
                        <div id="image-converter-embed"></div>
                    </div>
                    <script>
                        // Fetch and inject the image converter HTML (only the form part)
                        fetch('image_converter.html')
                            .then(r => r.text())
                            .then(html => {
                                // Extract only the .container inner HTML
                                const temp = document.createElement('div');
                                temp.innerHTML = html;
                                const container = temp.querySelector('.container');
                                if (container) {
                                    document.getElementById('image-converter-embed').innerHTML = container.innerHTML;
                                    // Re-run the script part for the converter
                                    const script = temp.querySelector('script');
                                    if (script) {
                                        eval(script.innerText);
                                    }
                                } else {
                                    document.getElementById('image-converter-embed').innerHTML = '<div class="alert alert-error">Kon de Image Converter niet laden.</div>';
                                }
                            });
                    </script>
                <?php endif; ?>
                                            ...
                                            <div class="border border-gray-200 rounded p-4 hover:shadow-md transition">
                                                <div class="admin-row flex justify-between items-start gap-4">
                                                    ...
                                                    <div class="flex-1">
                                                        ...
                                                        ?>
                                                        <p class="text-sm text-gray-600 mt-1"><strong>Wanneer:</strong> <?php echo htmlspecialchars($dateDisplay); ?><?php if ($timeDisplay) {
                                                                                                                                                                            echo ' ' . htmlspecialchars($timeDisplay);
                                                                                                                                                                        } ?><?php if ($timeEndDisplay) {
                                                                                                                                                                                echo ' - ' . htmlspecialchars($timeEndDisplay);
                                                                                                                                                                            } ?></p>
