...
try {
    ...
    $stmt = $pdo->prepare("SELECT * FROM events WHERE CONCAT(COALESCE(end_date, date), ' ', COALESCE(time_end, COALESCE(time, '23:59:59'))) > DATE_SUB(NOW(), INTERVAL 1 DAY) AND date <= DATE_ADD(CURDATE(), INTERVAL 14 DAY) ORDER BY date, time");
    $stmt->execute();
